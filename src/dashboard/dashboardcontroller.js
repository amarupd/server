...
from icsoftledger.dbo.Transactions t inner join icsoftledger.dbo.accounts a on a.ID = t.accountid inner join icsoftledger.dbo.accountheads ah on ah.AccountID = a.accountid
 ...
 and Month(t.transactiondate) = month([@ '{As On Date}' @]) and   Year(t.transactiondate) = Year(Dateadd(YY,0,'{As On Date}'))
...
from icsoftledger.dbo.Transactions t inner join icsoftledger.dbo.accounts a on a.ID = t.accountid inner join icsoftledger.dbo.accountheads ah on ah.AccountID = a.accountid
 ...
 and Month(t.transactiondate) = month('{As On Date}') and   Year(t.transactiondate) = Year(Dateadd(YY,-1,'{As On Date}'))
 Group By Ah.AccountName
 Union All 
 ----Current Quarter Sales 
...
from icsoftledger.dbo.Transactions t inner join icsoftledger.dbo.accounts a on a.ID = t.accountid inner join icsoftledger.dbo.accountheads ah on ah.AccountID = a.accountid
 ...
 (Select Quarter+'-'+yearname from icSoft.dbo.ST_FinancialYear Where '{As On Date}' between fromdate and ToDate )
 Group By Ah.AccountName
 Union All
...
from icsoftledger.dbo.Transactions t inner join icsoftledger.dbo.accounts a on a.ID = t.accountid inner join icsoftledger.dbo.accountheads ah on ah.AccountID = a.accountid
 Where ah.AccountTypeID = 4 -----and t.transactiondate between '01-Apr-24' and '30-Apr-24'
  and (Select Quarter+'-'+yearname from icSoft.dbo.ST_FinancialYear Where t.TransactionDate between fromdate and ToDate ) =
 (Select Quarter+'-'+yearname from icSoft.dbo.ST_FinancialYear Where Dateadd(YY,-1,'{As On Date}') between fromdate and ToDate )
 Group By Ah.AccountName
 Union All
 ---Current Year sales
...
from icsoftledger.dbo.Transactions t inner join icsoftledger.dbo.accounts a on a.ID = t.accountid inner join icsoftledger.dbo.accountheads ah on ah.AccountID = a.accountid
 ...
 (Select yearname from icSoft.dbo.ST_FinancialYear Where '{As On Date}' between fromdate and ToDate )
 Group By Ah.AccountName
...
from icsoftledger.dbo.Transactions t inner join icsoftledger.dbo.accounts a on a.ID = t.accountid inner join icsoftledger.dbo.accountheads ah on ah.AccountID = a.accountid
 Where ah.AccountTypeID = 4 -----and t.transactiondate between '01-Apr-24' and '30-Apr-24'
  and (Select yearname from icSoft.dbo.ST_FinancialYear Where t.TransactionDate between fromdate and ToDate ) =
 (Select yearname from icSoft.dbo.ST_FinancialYear Where Dateadd(YY,-1,'{As On Date}') between fromdate and ToDate )
 Group By Ah.AccountName
  ) t1
...
0 as NOWODBalOrdQty, 0 as NOWODBalOrdVal,0 as NOMODBalOrdQty,0 as NOMODBalOrdVal, 0 as BalOrdQty, 0 as BalOrderValue
 from ST_PendingSalesOrderdetail Where intDelDate < '{As On Date}'
Union All
 ------Duein Next 7 Days Sales Orders
...
Sum(OrderQty)-Sum(DespQty) as NOWODBalOrdQty, Sum(OrdValue)-Sum(DespValue) as NOWODBalOrdVal,0 as NOMODBalOrdQty,0 as NOMODBalOrdVal, 0 as BalOrdQty, 0 as BalOrderValue
 from ST_PendingSalesOrderdetail Where intDelDate between '{As On Date}' and Dateadd(dd,7,'{As On Date}')
Union All
 ------Duein Next One Month Days Sales Orders
 Select Count(POproductID) as NoofItems ,0 as BalOrdQty, 0 as BalOrdValue,
 ...
 from ST_PendingSalesOrderdetail Where intDelDate between '{As On Date}' and Dateadd(dd,30,'{As On Date}')
 Union All
...
left outer join ST_HTCondition ht on ht.HTID=rmadn.[HT Condition]
left outer join ST_SupplyCondition sc on sc.SCID = rmadn.[Supply Condition]
Where  Month(po.Created_Date) = month('{As On Date}') and   Year(po.Created_Date) = Year(Dateadd(YY,0,'{As On Date}'))
Union All
---Curr Month Prev Year Order Value
...
left outer join ST_HTCondition ht on ht.HTID=rmadn.[HT Condition]
left outer join ST_SupplyCondition sc on sc.SCID = rmadn.[Supply Condition]
Where  Month(po.Created_Date) = month('{As On Date}') and   Year(po.Created_Date) = Year(Dateadd(YY,-1,'{As On Date}'))
Union All
---Curr Quarter Order Value
...
left outer join ST_SupplyCondition sc on sc.SCID = rmadn.[Supply Condition]
Where  (Select Quarter+'-'+yearname from icSoft.dbo.ST_FinancialYear Where po.Created_Date between fromdate and ToDate ) =
 (Select Quarter+'-'+yearname from icSoft.dbo.ST_FinancialYear Where Dateadd(YY,0,'{As On Date}') between fromdate and ToDate )
Union All
---Curr Quarter Prev Year Order Value
...
left outer join ST_SupplyCondition sc on sc.SCID = rmadn.[Supply Condition]
Where (Select yearname from icSoft.dbo.ST_FinancialYear Where Convert(Date,po.Created_Date,11) between fromdate and ToDate ) =
 (Select yearname from icSoft.dbo.ST_FinancialYear Where Dateadd(YY,0,'{As On Date}') between fromdate and ToDate )
 Union All
 ---Prev Year Order Value
...
left outer join ST_SupplyCondition sc on sc.SCID = rmadn.[Supply Condition]
Where (Select yearname from icSoft.dbo.ST_FinancialYear Where Convert(Date,po.Created_Date,11) between fromdate and ToDate ) =
 (Select yearname from icSoft.dbo.ST_FinancialYear Where Dateadd(YY,-1,'{As On Date}') between fromdate and ToDate )) v
`;
...
select 0 as SubQty, 0 as SubValue, count(pp.QuotationProdId) as AcpQty, sum(qp.Qty*qp.Price_Unit) as AcpValue, 0 RejQty, 0 as RejValue
 from purchaseproduct pp inner join Purchase p on p.POID = pp.poid
 inner join quotation_Product qp on qp.QuotationProdID = pp.QuotationProdId
 where pp.QuotationProdId <> 0 and p.PODate > '{As On Date}' 
 UNION ALL
...
from quotation_Product qp inner join quotations q on qp.QuotID = q.QuotID
 where Price_Unit = 0 and QuotDate > '{As On Date}' ) v
`;
...
const enquiry_summary = async (req, res) => {
  const querry = `select 'Enquiry' Module,
	   count(case when Date_of_Enquiry = '{As On Date}' then IntEnqRefNo end) as  Today,
	   count(case when Date_of_Enquiry >= DATEADD(week, -1, '{As On Date}')then IntEnqRefNo end) as LastoneWeek,
	   count(case when Date_of_Enquiry >= DATEADD(month, -1, '{As On Date}')then IntEnqRefNo end) as LastoneMonth,
	   count(case when Date_of_Enquiry >= DATEADD(Year, -1, '{As On Date}')then IntEnqRefNo end) as LastoneYear ,
	   count(case when Date_of_Enquiry = '{As On Date}' and  enquirysentstatus = 'Hisotry' then IntEnqRefNo end) as  RejToday,
	   count(case when Date_of_Enquiry >= DATEADD(week, -1, '{As On Date}') and  enquirysentstatus = 'History' then IntEnqRefNo end) as RejLastoneWeek,
	   count(case when Date_of_Enquiry >= DATEADD(month, -1, '{As On Date}') and  enquirysentstatus = 'History'then IntEnqRefNo end) as RejLastoneMonth,
	   count(case when Date_of_Enquiry >= DATEADD(Year, -1, '{As On Date}') and  enquirysentstatus = 'History'then IntEnqRefNo end) as RejLastoneYear
	    from enquiry
`;
